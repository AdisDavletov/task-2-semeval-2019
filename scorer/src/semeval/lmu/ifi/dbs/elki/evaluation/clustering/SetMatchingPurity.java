...
import semeval.lmu.ifi.dbs.elki.evaluation.clustering.ClusterContingencyTable.Util;
...
public class SetMatchingPurity {
  ...
  protected SetMatchingPurity(ClusterContingencyTable table) {
   // super();
    double numobj = table.contingency[table.size1][table.size2];
      {
          smPurity = 0.0;
          smFFirst = 0.0;
          ...
          for (int i1 = 0; i1 < table.size1; i1++) {
              ...
              smFFirst += (table.contingency[i1][table.size2]
                      / table.contingency[table.size1][table.size2]) * fMax;
      ...
      smPurity /= numobj;
    {
      ...
      for(int i2 = 0; i2 < table.size2; i2++) {
        ...
        for(int i1 = 0; i1 < table.size1; i1++) {
          ...
          fMax = Math.max(fMax, (2.0 * table.contingency[i1][i2]) / (1.0*table.contingency[i1][table.size2] + table.contingency[table.size1][i2]));
          // / numobj));
        }
        smInversePurity += recallMax ;
        smFSecond += (table.contingency[table.size1][i2] / (1.0* table.contingency[table.size1][table.size2])) * fMax;
        // * contingency[i1][size2]/numobj;
      }
      smInversePurity /= numobj;
    }
  }
