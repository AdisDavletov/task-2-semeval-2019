...
public final class BitsUtil {
    ...
    /**
     * Convert bitset to a string consisting of "0" and "1", in high-endian
     * order.
     *
    ...
    /**
     * Convert bitset to a string consisting of "0" and "1", in low-endian
     * order.
     *
     * @param v Value to process
     * @return String representation
    ...
    public static double lpow2(long m, int n) {
        ...
        // Normal number.
        long bits = (shift > 0) ? (m >> shift) + ((m >> (shift - 1)) & 1)
                : // Rounding.
                m << -shift;
    ...
    public static double lpow10(long m, int n) {
        ...
        if (n >= 0) { // Positive power.
            ...
            pow2 -= shift;
            long mantissa = (shift < 0) ? (x3 << 31) | (x2 >>> 1)
                    : // x3 is 32 bits.
